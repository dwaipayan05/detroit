...
class VectorStoreRepoIngestor:
    ...
    def ingest(self) -> bool:
        ...
        if not _file_dict:
            ...
            return False
        self.logger.info(f"Parsed {_file_dict} files from the repository contents.")
    ...
    def _ingest_into_vector_store(self, file_dict: dict) -> bool:
        ...
        try:
            ...
            if qdrant.get_collection(collection_name=f"{self.repo_owner}.{self.repo_name}"):
                self.logger.info(f"Collection {self.repo_owner}.{self.repo_name} already exists, Skipping Ingestion")
                return True
            ...
            for file_name, metadata in file_dict.items():
                ...
                if not documents:
                    self.logger.warning(f"No documents created for {file_name}.")
                    continue

                self.logger.info(f"Created {len(documents)} documents for {file_name}.")
                vector_store.add_documents(documents)
                self.logger.info(f"Successfully Added Documents to the Vector Store for {file_name}")
    ...
    def _ingest_git_repo(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Starting ingestion for {self.repo_owner}/{self.repo_name}")
            summary, tree, content = ingest(repo_url)

            directory = os.path.join(os.getcwd() + "/tmp", self.repo_owner, self.repo_name)
    ...
    def _parse_repo_contents(self) -> dict:
        ...
        content_file = os.path.join(os.getcwd() + "/tmp", self.repo_owner, self.repo_name, 'content.txt')
        ...
        for chunk in chunk_read(content_file):
            ...
            for line in _lines:
                ...
                if line == boundary_line:
                    self.logger.debug(f"Found boundary line, current state: {file_marker}")
                    if True not in file_marker:
                        ...
                        _file_path = os.path.join(os.getcwd() + "/tmp", self.repo_owner, self.repo_name, "sourcefiles/", file_name)
                ...
                if file_marker[0] and not file_marker[1] and line.startswith("FILE: "):
                    self.logger.debug(f"Found file marker: {line}, current state: {file_marker}")
                    file_name = line[len("FILE: "):].strip()
                    file_marker[1] = True
                    continue  # Skip marker line

                if file_marker[0] and file_marker[1] and file_marker[2]:
                    self.logger.debug(f"Appending line to file content: {line}")
        ...
        self.logger.info(f"Parsed {len(file_dict)} files from the repository contents.")
        return file_dict