...
def main():
    ...
    while True:
        ...
        if item:
            ...
            logger.info(json.dumps(metadata, sort_keys=True, indent=4, default=str))
            ...
            mongodb_service.update_repo_metadata(repo_url, status)

            logger.info(f"Repo Ingestion Status: {status}")

            # TODO: Remove this block, this is only for ease of testing
            time.sleep(10)
            mongodb_service.delete_repo_metadata(repo_url)

            mongodb_service.close()

            logger.info(f"Completed Processing Repo Name: {_name}")
